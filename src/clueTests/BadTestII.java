...
public class BadTestII {
    ...
    /*
     * @Test public void testAdjacency() { // Locations with only walkways as
     * adjacent locations Map<Character, String> rooms = board.getRooms();
     * LinkedList<BoardCell> testList = board.getAdjList(8, 13);
     * Assert.assertEquals(2, testList.size()); // Locations that are at each
     * edge of the board testList = board.getAdjList(11, 0);
     * Assert.assertEquals(2, testList.size()); testList = board.getAdjList(22,
     * 7); Assert.assertEquals(2, testList.size()); testList =
     * board.getAdjList(16, 13); Assert.assertEquals(1, testList.size());
     * testList = board.getAdjList(0, 0); Assert.assertEquals(3,
     * testList.size()); // Locations that are beside a room cell that is not a
     * doorway testList = board.getAdjList(11, 0); Assert.assertEquals(2,
     * testList.size()); testList = board.getAdjList(22, 7);
     * Assert.assertEquals(2, testList.size()); testList = board.getAdjList(16,
     * 13); Assert.assertEquals(1, testList.size()); // Locations that are
     * adjacent to a doorway with needed direction testList =
     * board.getAdjList(7, 0); Assert.assertEquals(2, testList.size()); testList
     * = board.getAdjList(4, 9); Assert.assertEquals(4, testList.size());
     * testList = board.getAdjList(16, 10); Assert.assertEquals(4,
     * testList.size()); testList = board.getAdjList(21, 7);
     * Assert.assertEquals(3, testList.size()); // Locations that are doorways
     * testList = board.getAdjList(14, 2); Assert.assertEquals(1,
     * testList.size()); testList = board.getAdjList(2, 2);
     * Assert.assertEquals(1, testList.size()); }
     * 
     * @Test public void testTargets() { // Targets along walkways, at various
     * distances board.calcTargets(9, 4, 1); Set<BoardCell> targets =
     * board.getTargets(); Assert.assertEquals(3, targets.size());
     ...
     * 
     * board.calcTargets(9, 4, 2); targets = board.getTargets();
     ...
     * 
     * board.calcTargets(11, 9, 3); targets = board.getTargets();
     ...
     * 
     * board.calcTargets(11, 9, 2); targets = board.getTargets();
     * Assert.assertEquals(4, targets.size());
     ...
     * 
     * // Targets that allow the user to enter a room board.calcTargets(3, 3,
     * 2); targets = board.getTargets();
     * Assert.assertTrue(targets.contains(board.getCellAt(2, 2)));
     * 
     * board.calcTargets(2, 11, 2); targets = board.getTargets();
     ...
     * targets = board.getTargets(); Assert.assertEquals(1, targets.size());
     * Assert.assertTrue(targets.contains(board.getCellAt(6, 0)));
     * 
     * board.calcTargets(16, 11, 2); targets = board.getTargets();
    ...
    // System.out.println(board.getAdjList(5, 15));
    // Assert.assertEquals(1, testList.size());
    ...
    // System.out.println(board.getAdjList(4, 4));
    ...
    // @Test
    // public void testTargetsOther() {
    // board.calcTargets(0, 5, 2);
    // Set<BoardCell> targets = board.getTargets();
    // System.out.println(targets);
    // Assert.assertEquals(3, targets.size());
    // Assert.assertTrue(targets.contains(board.getCellAt(0,6)));
    // Assert.assertTrue(targets.contains(board.getCellAt(1,5)));
